...
function getAreaReading(area) {
  ...
  const readGroups = (matchedGroups) => matchedGroups.map(([, value, separator, separatorIndex]) => {
  ...
  if (groups) return readGroups(groups);

  // JMAは「熊本県宇城市」のように、辞書にある地域名を連結した表記を返すことがある。
  // 完全一致がなくても、最長一致で各地域名を分割して読みを組み立てる。
  const text = String(area || '');
  const readings = [];
  for (let index = 0; index < text.length;) {
    const name = areaNames.find((candidate) => text.startsWith(candidate, index));
    if (!name) return null;
    readings.push(readGroups(areaRubyGroups[name]));
    index += name.length;
  }
