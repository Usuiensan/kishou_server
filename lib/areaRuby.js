...
function getAreaReading(area) {
  ...
  const reading = AREA_READING_OVERRIDES[area] || groups.map(([, value]) => value).join('');
  if (AREA_READING_OVERRIDES[area]) return reading;
  const prefecture = prefectureNames.find((name) => name !== area && area.startsWith(name));
  if (prefecture && areaRubyGroups[prefecture]) {
    const prefectureReading = areaRubyGroups[prefecture].map(([, value]) => value).join('');
    if (reading.startsWith(prefectureReading)) {
