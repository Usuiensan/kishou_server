...
function shouldDeliver(formatted, config) {
  ...
  if (formatted.type === 'eew') return categories.includes('default') || categories.includes('eew');
  if (categories.includes('default')) return intensityValue(formatted.type) >= 4;
...
module.exports = { createDiscordBot, loadSubscriptions, saveSubscriptions, intensityValue, shouldDeliver };
