...
const { filterFormattedEarthquakeByIntensity } = require('./formatter');
...
const SUBSCRIPTION_CATEGORIES = [
  ...
  ...Array.from({ length: 7 }, (_, index) => `include_intensity_${index + 1}`),
  'eew',
  'nerv_tornado',
  'nerv_level4',
  'nerv_level5',
...
function getDisplayIntensityThreshold(config) {
  const values = (config?.categories || [])
    .filter((category) => category.startsWith('include_intensity_'))
    .map((category) => Number(category.split('_')[2]))
...
function createDiscordBot({ token = process.env.DISCORD_BOT_TOKEN, clientId = process.env.DISCORD_CLIENT_ID, logger = console } = {}) {
  ...
  const categoryOptions = [
    ...
    ...Array.from({ length: 7 }, (_, index) => ({ label: `表示・震度${index + 1}以上`, value: `include_intensity_${index + 1}` })),
  ...
  client.on('interactionCreate', async (interaction) => {
    ...
    if (interaction.isStringSelectMenu() && interaction.customId.startsWith('nerv:')) {
      ...
      const thresholdCategories = [...current].filter((category) => category.startsWith('intensity_') && !category.startsWith('include_'));
      const displayCategories = [...current].filter((category) => category.startsWith('include_intensity_'));
      const nervCategories = [...current].filter((category) => category.startsWith('nerv_'));
      await interaction.update({ content: `✅ 購読カテゴリを更新しました: 閾値=${thresholdCategories.join(', ') || 'なし'} / 表示=${displayCategories.join(', ') || '全震度'} / NERV=${nervCategories.join(', ') || 'なし'}`, components: [] });
      return;
    }
    ...
    if (interaction.commandName === 'subscribe' || interaction.commandName === 'unsubscribe') {
      ...
      if (!config) {
        await interaction.reply('ℹ️ 未設定（購読なし）');
      } else {
        const thresholdCategories = config.categories.filter((category) => category.startsWith('intensity_') && !category.startsWith('include_'));
        const displayCategories = config.categories.filter((category) => category.startsWith('include_intensity_'));
        const nervCategories = config.categories.filter((category) => category.startsWith('nerv_'));
        await interaction.reply(`✅ 購読中: 閾値=${thresholdCategories.join(', ') || 'なし'} / 表示=${displayCategories.join(', ') || '全震度'} / NERV=${nervCategories.join(', ') || 'なし'}`);
      }
    }
  });
    ...
    async send(formatted) {
      for (const channels of Object.values(subscriptions)) {
        for (const [channelId, config] of Object.entries(channels)) {
          ...
          const displayIntensityThreshold = getDisplayIntensityThreshold(config);
          const displayFormatted = filterFormattedEarthquakeByIntensity(formatted, displayIntensityThreshold);
          const message = buildDiscordDebugMessage(displayFormatted);
          const chunks = splitDiscordMessageByLines(message);
          const deliveryKey = createDiscordDeliveryKey({
            ...displayFormatted,
            displayIntensityThreshold,
          });
