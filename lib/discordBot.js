...
const { buildDiscordDebugMessage, splitDiscordMessage } = require('./discordWebhook');
    ...
    async send(formatted) {
      ...
      for (const channelIds of Object.values(subscriptions)) {
        for (const channelId of channelIds) {
          ...
          for (const chunk of splitDiscordMessage(message)) await channel.send({ content: chunk });
        }
      }
    },
    client,
