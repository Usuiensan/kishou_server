...
function wrapIntensityAreas(prefix, areas) {
  ...
  const wrapped = [];
  const firstAreaOnly = areas.length >= 2
    && getDiscordDisplayWidth(`${prefix}${areas[0]}${DISCORD_AREA_SEPARATOR}${areas[1]}`) > DISCORD_INTENSITY_FIRST_LINE_WIDTH;
  if (firstAreaOnly) {
    wrapped.push(prefix);
  }

  let current = firstAreaOnly ? areas[0] : prefix;
  let currentHasArea = firstAreaOnly;
  let lineLimit = firstAreaOnly ? DISCORD_INTENSITY_LINE_WIDTH : DISCORD_INTENSITY_FIRST_LINE_WIDTH;

  for (const area of firstAreaOnly ? areas.slice(1) : areas) {
...
function buildDiscordDebugMessage(formatted) {
  ...
  return `${toFullwidthDigits(body)}${body ? '\n\n' : ''}${metadata}`;
}
