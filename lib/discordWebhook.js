...
function formatLinesForDebug(formatted) {
  ...
  for (const line of sourceLines) {
    ...
    if (longPeriodMatch) {
      ...
      const areaText = longPeriodMatch[2].trim().split(/\s+/).filter(Boolean).join(' ');
    ...
    if (pendingLegacyLongPeriodKey) {
      const areaText = line.trim().split(/\s+/).filter(Boolean).join(' ');
    ...
    const areaText = match[2].trim().split(/\s+/).filter(Boolean).join(' ');
  ...
  if (longPeriodLines.size > 0) {
    const compact = [...longPeriodLines.entries()]
      .map(([key, areas]) => `- **【長周期地震動】${key}**：${areas.join(' ')}`);
    lines.push(...compact);
  }
