...
const { getAreaReading, toHalfwidthKatakana } = require('./areaRuby');
...
function formatAreaItems(prefix, areas) {
  return areas.map((area) => {
    const hiraganaReading = getAreaReadingForDisplay(area);
    const hiraganaItem = `- ${prefix}${area}${DISCORD_AREA_SEPARATOR}${hiraganaReading}`;
    const reading = getDiscordDisplayWidth(hiraganaItem) > DISCORD_PRINT_LINE_WIDTH
      ? toHalfwidthKatakana(hiraganaReading)
      : hiraganaReading;
    const item = `- ${prefix}${area}${DISCORD_AREA_SEPARATOR}${reading}`;
    return item;
  });
}
...
function formatLinesForDebug(formatted) {
  ...
  for (const line of sourceLines) {
    ...
    const match = line.match(/^(?:\*\*|__)?震度([7654321７６５４３２１](?:弱|強)?)(?:\*\*|__)?\s*(.*)$/);
  ...
  if (intensityLines.size > 0) {
    ...
    for (const [key, areas] of intensityLines.entries()) {
      const prefix = `**${key}**`;
      compact.push(prefix, ...formatAreaItems('', areas));
    }
  ...
  if (longPeriodLines.size > 0) {
    const compact = [...longPeriodLines.entries()]
      .flatMap(([key, areas]) => [`**【長周期地震動】${key}**`, ...formatAreaItems('', areas)]);
    lines.push(...compact);
  }
...
function buildDiscordDebugMessage(formatted) {
  ...
  if (formatted.isEEW && formatted.eewHeadline) {
    ...
    body = [
      ...
      ...formatAreaItems('', primaryAreas),
      ...(additionalAreas.length > 0 ? formatAreaItems('追加：', additionalAreas) : []),
    ].filter(Boolean).join('\n');
  }
