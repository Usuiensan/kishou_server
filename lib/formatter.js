...
function formatEarthquake(eq) {
  ...
  if (eqInfo && eqInfo.hypocenter) {
    const magStr = eqInfo.magnitude === 'NaN' ? eqInfo.magnitudeDescription || '不明' : `マグニチュード${eqInfo.magnitude}`;
  ...
  if (eq.intensity && eq.intensity.maxInt) {
    ...
    intensityOrder.forEach((int) => {
      ...
      if (areas && areas.length > 0) {
        const label = `<u>震度${INTENSITY_MAP[int] || int}</u>`;
