...
function createLineObjects(lines, firstLineDuration = 10.0) {
  return lines.map((line, index) => {
    // 最初の行は指定された時間、それ以外は一律 10.0 秒
    const duration = index === 0 ? firstLineDuration : 10.0;
...
function formatEarthquake(eq) {
  ...
  if (eq.isEEW) {
    ...
    return {
      ...
      id: eq.eventId,
      isEEW: true,
      lines: createLineObjects([packedText], 3.5),
    };
  }
