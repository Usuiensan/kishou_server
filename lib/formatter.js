...
function createLineObjects(lines) {
  return lines.map((line) => {
    // タグ（<...>）を除去して純粋な文字数をカウント
    const visibleText = line.replace(/<[^>]*>/g, '').replace(/\n/g, '');
    // 1秒4文字計算（最低3秒）
    const calculatedDuration = Math.max(3, visibleText.length / 2);

    // 小数点第1位までに丸める
    const duration = Math.round(calculatedDuration * 10) / 10;
...
function formatEarthquake(eq) {
  ...
  if (eq.isEEW) {
    ...
    return {
      type: 'emergency',
      id: eq.eventId,
      lines: createLineObjects([packedText]),
    };
  }
  ...
  lines.push(`${timeStr}${regionPrefix}地震がありました`);
  ...
  if (eq.lgIntensity && eq.lgIntensity.maxLgInt) {
    ...
    lgIntensityOrder.forEach((lgInt) => {
      ...
      if (areas && areas.length > 0) {
        ...
        lines.push(...splitAreasHelper(label, areas, 6, '12em'));
      }
    });
  }
