...
function createLineObjects(lines, firstLineDuration = 7.5) {
  return lines.map((line, index) => {
    ...
    return { text: toUnityDisplayText(line), duration: duration };
  });
}
...
function formatEarthquake(eq) {
  ...
  if (eq.isEEW) {
    ...
    return {
      ...
      id: eq.eventId,
      isEEW: true,
      eewHeadline: headline,
      eewAreas: eq.eewAreas || [],
      eewAdditionalAreas: eq.eewAdditionalAreas || [],
  ...
  // 震度1以上のJMA電文はすべて通知対象にする。
  if (maxInt && !isIntensityAtLeast(maxInt, '1') && !eq.lgIntensity?.maxLgInt) {
    return null;
  }
  if (!maxInt && !eq.lgIntensity?.maxLgInt) {
    const magnitude = parseFloat(eq.earthquake?.magnitude);
    if (!Number.isFinite(magnitude) || magnitude < 5.0) return null;
  }
  ...
  let comment = (eq.comment || '')
    ...
    .replace(/（大津波警報・津波警報あるいは津波注意報）/g, '')
    .replace(/各長周期地震動階級に対する簡易な現象表現[\s\S]*?ご活用ください。?/g, '')
  ...
  if (eq.intensity && eq.intensity.maxInt) {
    ...
    // 通知判定は最大震度4以上を維持しつつ、通知対象となった地震では
    // 震度1以上の地域をすべて表示する。
...
// 沖合観測点は同じ海域・距離に複数の観測点があるため末尾に A/B などの
// 識別子が付くが、一般向け表示では津波の発生・到達を優先して省略する。
function simplifyOffshoreObservationName(name) {
  return String(name || '').replace(/[Ａ-ＺA-Z]$/, '');
}
...
function formatTsunami(ts) {
  ...
  if (ts.title?.includes('沖合の津波観測') || ts.tsunami.offshoreObservations?.length > 0) {
    ...
    ts.tsunami.offshoreObservations.forEach((observation) => {
      ...
      lines.push(`【観測】<nobr>${simplifyOffshoreObservationName(observation.name)}</nobr>\n最大 ${height}  ${time}`);
    });
  ...
  ts.tsunami.forecasts.forEach((f) => {
    ...
    if (color) {
      ...
      if (f.firstHeight?.condition) {
        // 到達時刻の代わりに、気象庁が発表する状態を優先表示する。
        // 「すでに到達と予測」「第一波到達を確認」などの表現も落とさない。
        const condition = String(f.firstHeight.condition);
        if (time === '不明' || /来襲|到達|確認|推測/.test(condition)) {
          time = condition;
        }
      }
      time = time
        .replace('ただちに津波来襲と予測', 'すぐ来る')
        .replace('津波到達中と推測', '到達か')
        .replace('すでに津波到達と予測', 'すでに到達と予測');
      lines.push(`<color=${color}>【${kind}】</color><nobr>${f.area.name}</nobr>\n【予想】  ${f.maxHeight}  ${time}`);
    }
  });

  if (ts.intensity && ts.intensity.maxInt && isIntensityAtLeast(ts.intensity.maxInt, '1')) {
    lines.push('');
    ...
    const intensityOrder = ['7', '6+', '6-', '5+', '5-', '4'];
    intensityOrder.forEach((int) => {
      const areas = targetGroups?.[int];
      if (areas && areas.length > 0) {
        const intStr = INTENSITY_MAP[int] || int;
...
function filterFormattedEarthquakeByIntensity(formatted, minimumIntensity = null) {
  if (!formatted || minimumIntensity == null || !String(formatted.type || '').startsWith('earthquake')) return formatted;
  const minimum = Number(minimumIntensity);
  if (!Number.isFinite(minimum)) return formatted;
  const filteredLines = (formatted.lines || []).filter((line) => {
    const text = typeof line === 'string' ? line : line?.text || '';
    const match = text.match(/<(?:u|b)>震度([1-7１２３４５６７])(?:弱|強)?<\/(?:u|b)>/);
    if (!match) return true;
    const value = Number(String(match[1]).replace(/[１２３４５６７]/g, (digit) => '１２３４５６７'.indexOf(digit) + 1));
    return value >= minimum;
  });
  return { ...formatted, lines: filteredLines };
}

module.exports = {
  formatEarthquake,
  filterFormattedEarthquakeByIntensity,
  formatTsunami,
  formatWeather,
};
