...
function formatEarthquake(eq) {
  ...
  if (eqInfo && eqInfo.hypocenter) {
    ...
    if (desc.includes('ごく浅い')) {
      ...
      // 「深さは６００ｋｍ以上」や「深さ　１０ｋｍ」などのパターンを抽出
    ...
    // description から抽出できなかった場合のフォールバック: 座標からの抽出 (ISO6709)
    if (depth === '不明') {
      // 深度が [+-]0/ の場合は「ごく浅い」とする。また [+-] を許容する
  ...
  // どちらもない場合
  if (!eq.intensity && (!eq.lgIntensity || !eq.lgIntensity.maxLgInt) && eq.earthquake) {
    // 震度もなく、震源情報のみの場合
    // lines.push('この地震による各地の震度は調査中です');
  }
  ...
  ts.tsunami.forecasts.forEach((f) => {
    ...
    if (color) {
      ...
      if (f.firstHeight?.condition) {
        // 時刻が不明、または特定の緊急キーワードが含まれる場合は Condition を優先する
