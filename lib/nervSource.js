...
function getNervCategories(status) {
  const text = decodeHtml(status.content || '');
  if (isTornadoAlert({ ...status, content: text })) return ['nerv_tornado'];

  const categories = new Set();
  if (/レベル\s*[５5]/.test(text)) categories.add('nerv_level5');
  else if (/レベル\s*[４4]/.test(text)) categories.add('nerv_level4');
  if (/ニュース|NHK|速報/.test(text)) categories.add('nerv_news');

  const baseCategory = classifyNervStatus({ ...status, content: text });
  categories.add(`nerv_${baseCategory}`);
  return [...categories];
}

function isNervRelevantStatus(status) {
  const text = decodeHtml(status.content || '');
  if (isDuplicateEarthquakeSource(status) || isExcludedNervStatus(status)) return false;
...
function normalizeStatus(status, account = DEFAULT_ACCOUNT) {
  ...
  return {
    ...
    nervCategories: getNervCategories({ ...status, content: decodedContent }),
