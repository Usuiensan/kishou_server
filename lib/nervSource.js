...
function isDuplicateEarthquakeSource(status) {
  const text = `${status.content || ''} ${(status.tags || []).map((tag) => tag.name).join(' ')}`;
  return /#?緊急|緊急地震速報|震度速報|地震情報|津波|大津波警報|津波警報|津波注意報/.test(text);
}

function normalizeStatus(status, account = DEFAULT_ACCOUNT) {
  const content = decodeHtml(status.content);
  return {
    type: 'nerv',
    id: `nerv_${status.id}`,
    source: 'nerv',
    ...
    nervCategory: classifyNervStatus({ ...status, content }),
    lines: [{ text: content, duration: 10 }],
  };
}

async function fetchNervStatuses({ fetchImpl, baseUrl = DEFAULT_BASE_URL, account = DEFAULT_ACCOUNT, sinceId = null, limit = 20 }) {
  ...
  const normalized = statuses
    .filter((status) => status && status.id && status.content && !isDuplicateEarthquakeSource(status))
