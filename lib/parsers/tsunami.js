...
function parseTsunami(xml) {
    ...
    if (body.Tsunami?.Observation) {
        ...
        result.tsunami.offshoreObservations = items.flatMap(item => {
            const stations = item.Station
                ? (Array.isArray(item.Station) ? item.Station : [item.Station])
                : [];
            ...
            return stations.map(station => ({
                ...
                firstHeight: station.FirstHeight ? {
                    arrivalTime: station.FirstHeight.ArrivalTime || '',
                    formattedArrivalTime: formatTime(station.FirstHeight.ArrivalTime),
                    initial: station.FirstHeight.Initial || '',
                    condition: station.FirstHeight.Condition || '',
                } : null,
                maxHeight: {
                    dateTime: station.MaxHeight?.DateTime || '',
                    formattedDateTime: formatTime(station.MaxHeight?.DateTime),
                    condition: station.MaxHeight?.Condition || '',
                    value: station.MaxHeight?.['jmx_eb:TsunamiHeight']?.['#text'] || station.MaxHeight?.['jmx_eb:TsunamiHeight'] || '',
                    description: station.MaxHeight?.['jmx_eb:TsunamiHeight']?.description || '',
                },
            }));
        });
    }

    // 沖合観測から推定される沿岸での津波到達時刻・高さ
    if (body.Tsunami?.Estimation) {
        const estimation = body.Tsunami.Estimation;
        const items = Array.isArray(estimation.Item) ? estimation.Item : [estimation.Item];

        result.tsunami.estimations = items.map(item => ({
            area: {
                name: item.Area.Name,
                code: item.Area.Code,
            },
            ...
            maxHeight: {
                dateTime: item.MaxHeight?.DateTime || '',
                formattedDateTime: formatTime(item.MaxHeight?.DateTime),
                condition: item.MaxHeight?.Condition || '',
                value: item.MaxHeight?.['jmx_eb:TsunamiHeight']?.['#text'] || item.MaxHeight?.['jmx_eb:TsunamiHeight'] || '',
                description: item.MaxHeight?.['jmx_eb:TsunamiHeight']?.description || '',
            },
        }));
    }
...
module.exports = { parseTsunami, formatTime };
